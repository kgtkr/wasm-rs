...
impl Cmd {
    ...
    fn json_to_value(json: &Value) -> Val {
        ...
        match json_obj.get("type").unwrap().as_str().unwrap() {
            "i32" => Val::I32(
                (BigInt::from_str(json_obj.get("value").unwrap().as_str().unwrap()).unwrap()
                    & BigInt::from_i32(i32::max_value()).unwrap())
                .to_i32()
                .unwrap(),
            ),
            "i64" => Val::I64(
                (BigInt::from_str(json_obj.get("value").unwrap().as_str().unwrap()).unwrap()
                    & BigInt::from_i64(i64::max_value()).unwrap())
                .to_i64()
                .unwrap(),
            ),
            "f32" => Val::F32(
    ...
    for cmd in &cmds {
        match cmd {
            ...
            Cmd::AssertReturn {
                ...
                println!("[begin assert-return]{}", name);
            ...
            Cmd::Skip(name) => println!("[skip]{}", name),
        }
    }
}
